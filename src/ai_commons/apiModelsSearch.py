...
class Document(BaseModel):
    ...
    def from_json_directory(cls, directory: str) -> List["Document"]:
        documents = []
        files = get_files(
            directory, patterns_to_match=["*.json"], patterns_to_ignore=["*index.json"]
        )
        for file in files:
            documents.append(cls.from_json_file(os.path.join(directory, file)))
        return documents
    ...
    def to_lc_document(self) -> lc_Document:
        return lc_Document(page_content=self.content, metadata=self.get_metadata())
...
class Chunk(Document):
    ...
    @classmethod
    def chroma_chunks2chunk_array(
        cls, chroma_chunks, search_term: str = None
    ) -> list["Chunk"]:
        chunks = []
        for counter, id in enumerate(chroma_chunks["ids"][0]):
            ...
            if search_term:
                search_info = SearchInfo(
                    ...
                    distance=chroma_chunks["distances"][0][counter],
                )
    ...
    def from_json_file(cls, file_path: str) -> "Chunk":
        ...
        if "original_document_id" not in chunk_dict:
            raise ValueError(
                "The JSON file must contain a 'original_document_id' field. File:"
                f" {file_path}"
            )
