...
class ChainFactory:
    ...
    def __init__(self, chains_index_file: str = CHAINS_INDEX_FILE):
        if not os.path.exists(chains_index_file):
            logger.warning(
                f"Chains index file not found: {chains_index_file}, using default"
                " chains."
            )
        ...
        # If we have a default_chain name in the index file, we need to copy over
        # the parameters of that chain into the §default_chain"
        if "default_chain" in self.chains_index.keys():
            ...
            if default_chain_id not in self.chains_index:
                raise ValueError(
                    f"Default chain ID {default_chain_id} not found in chains index."
                )
            else:
            ...
            self.chains_index["default_chain"] = self.chains_index[
                next(iter(self.chains_index))
            ]
    ...
    def _create_default_chains_index(self):
        ...
        for chain_type in CHAIN_TYPES:
            chains_index[chain_type] = {"chain_type": chain_type}
        return chains_index
    ...
    def create_chain(self, parameters: Dict[str, Any]) -> Chain:
        ...
        logger.info(
            "Creating chain with parameters (after setting default chain type):"
            f" {parameters}"
        )
    ...
    def create_chain_by_id(self, chain_id: str) -> Chain:
        ...
        logger.info(
            f"Creating chain with parameters: {chain_parameters} taken from index"
            f" {json.dumps(self.chains_index, indent=2)}"
        )
