...
from ai_commons.constants import (
    CHUNKER_SEPARATOR,
    CHUNKER_CHUNK_SIZE,
    CHUNKER_CHUNK_OVERLAP,
)
...
class ChunkerCharacterTextSplitter(Chunker):
    ...
    def __init__(self, parameters: Dict[str, Any]):
        ...
        self.parameters["chunk_size"] = parameters.get("chunk_size", CHUNKER_CHUNK_SIZE)
    ...
    def _chunkify_document(self, document: Document) -> List[Chunk]:
        ...
        for lc_chunk in lc_chunks:
            ...
            chunks.append(chunk)
        logger.info(
            f"Chunkified a document of length {len(document.content)} in"
            f" {len(chunks)} chunks of length:"
            f" {[len(chunk.content) for chunk in chunks]}"
        )
        return chunks
