...
class Brain:
    ...
    @classmethod
    def get_brain_parameters_list(
        cls, brains_index_file: str = AI_BRAINS_INDEX_FILE
    ) -> List[BrainParameters]:
        logger.info(
            "get_brain_parameters_list: Reading from brains index file:"
            f" {brains_index_file}"
        )
        brain_dict = read_dict_from_file(full_filename=brains_index_file)
        logger.info(f"Brain dict: {brain_dict}")
        brain_parameters = [
            BrainParameters(brain_id=brain_id, **brain)
            for brain_id, brain in brain_dict.items()
        ]
        return brain_parameters

    @classmethod
    def is_valid_brain_id(
        cls, brain_id: str, brains_index_file: str = AI_BRAINS_INDEX_FILE
    ) -> bool:
    ...
    @classmethod
    def get_brain_by_id(
        cls, brain_id: str, brains_index_filename: str = AI_BRAINS_INDEX_FILE
    ):
        brain_parameters_list = cls.get_brain_parameters_list(brains_index_filename)
    ...
    def get_default_brain(cls, brains_index_file: str = AI_BRAINS_INDEX_FILE):
        logger.info(
            f"get_default_brain: Reading from brains index file: {brains_index_file}"
        )
        brain_list = cls.get_brain_parameters_list(brains_index_file)
        ...
        for brain_parameter in brain_list:
            if getattr(brain_parameter, "default", False):
                ...
                break
        return cls.get_brain_by_id(
            default_brain_id, brains_index_filename=brains_index_file
        )
    ...
    def __init__(self, parameters: BrainParameters, id: str = None):
        ...
        self.parameters["allow_duplicates"] = self.parameters.get(
            "allow_duplicates", False
        )
        self.parameters["max_no_of_docs"] = self.parameters.get("max_no_of_docs", 0)
        self.parameters["embedding_function"] = self.parameters.get(
            "embedding_function", {}
        )
        ...
        self.chroma_client = chromadb.PersistentClient(
            path=self.parameters["chroma_directory"],
            settings=Settings(anonymized_telemetry=False),
        )
        embedding_function = EmbeddingFunctionFactory().create_embedding_function(
            parameters=self.parameters["embedding_function"]
        )
        self.chroma_collection = self.chroma_client.get_or_create_collection(
            name=AI_BRAIN_COLLECTION_NAME, embedding_function=embedding_function
        )
        ...
        logger.info(
            f"Brain initialized. Data directory: {data_directory}, no of document:"
            f" {self.number_of_documents()}, no of chunks: {self.number_of_chunks()}"
        )
    ...
    def get_scraper(self) -> BrainScraper:
        if not self.parameters.get("scraper"):
            ...
            return None
        return BrainScraperFactory().create_brain_scraper(
            parameters=self.parameters["scraper"]
        )
    ...
    def get_parameters(self) -> Dict[str, Any]:
        ...
        sorted_parameters = OrderedDict(sorted(self.parameters.items()))
    ...
    def get_parameters_and_statistics(self) -> Dict[str, Any]:
        ...
        parameters_and_statistics = {**parameters, **statistics}
    ...
    def __str__(self):
        return (
            f"Brain with {self.number_of_documents()} documents and"
            f" {self.number_of_chunks()} chunks, stored in"
    ...
    def _write_index_to_file(self):
        write_dict_to_file(
            dictionary=self.document_index,
        ...
        chunks_metadatas = [
            {
                "title": chunk.title,
                "uri": chunk.uri,
                "original_document_id": chunk.original_document_id,
            }
            for chunk in chunks
        ]
        chunks_ids = [chunk.id for chunk in chunks]
        for i in tqdm(
            range(0, len(chunks_content), batch_size),
            desc=f"Adding {len(chunks)} chunks to ChromaDB in batches of {batch_size}",
        ):
            ...
            self.chroma_collection.add(
                documents=chunks_content_batch, metadatas=batch_metadatas, ids=batch_ids
            )
    ...
    def import_chunks_from_directory(self):
        ...
        if not self.parameters.get("chunks_directory"):
            raise ValueError(
                "chunks_directory not given, brain is not be able to load."
            )
        ...
        if not os.path.exists(self.parameters["chunks_directory"]):
            ...
            raise FileNotFoundError(
                ...
                f" {self.parameters['chunks_directory']}"
            )
        # Read chunks from directory
        files = get_files(
            self.parameters["chunks_directory"],
            patterns_to_match=["*.json"],
            patterns_to_ignore=["*index.json"],
        )
        chunks = []
        for file in tqdm(
            files, desc=f"Reading chunks from {self.parameters["chunks_directory"]}"
        ):
    ...
    def import_document(self, document: Document):
        if (
            self.parameters["max_no_of_docs"] > 0
            and self.number_of_documents() >= self.parameters["max_no_of_docs"]
        ):
            logger.warning(
                f"Max number of documents reached: {self.parameters['max_no_of_docs']}."
                " Document not added."
            )
            return
        if len(document.content) == 0:
            logger.warning(
                f"Document with empty content cannot be added. URI: {document.uri}"
            )
            return
        ...
        if not self.parameters["allow_duplicates"]:
            if self.is_in_by_uri(document.uri):
                logger.warning(
                    f"Document with URI {document.uri} already exists in the"
                    " collection."
                )
                return
