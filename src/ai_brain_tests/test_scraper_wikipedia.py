...
class TestWikipediaScraper(unittest.TestCase):
    ...
    def test_save_page_as_doc(self):
        parameters = {"start_page_title": WIKIPEDIA_PAGE, "target_dir": TMP_DATA_DIR}
        scraper = BrainScraperWikipedia(parameters=parameters)
        test_dir = os.path.join(
            TMP_DATA_DIR,
            simplify_text(get_now_as_string() + "_test_wikipedia_save_page"),
        )
    ...
    def test_scraper_by_brain_definition(self):
        brain = Brain.get_brain_by_id(
            "scraper_test",
            brains_index_filename=os.path.join(TEST_DATA_DIR, "brains.json"),
        )
